...
### 객체지향 특징
- **추상화(Abstraction)** : 공통의 속성이나 기능을 한곳에 모은 것
- **캡슐화(Encapsulation)** : 비슷한 역할을 하는 속성과 메서드를 하나의 클래스로 모은 것
  - 정보은닉 : 내부의 로직이나 변수들을 감추고 외부에는 기능(api)만 제공하는 것
- **상속(Inheritance)** : 재사용을 목적으로, 상위 개념의 특징을 하위 개념이 물려받는 것
- **다형성(Polymorphism)** : 같은 모양의 함수가 상황에 따라 다르게 동작 하는 것
